...
function getFilesToBackup(lastBackup, changes) {
  let changesFiltered = changes.filter((x) => x[1] > lastBackup);
  let changesIds = changesFiltered.map((x) => x[0]);
  let changesSorted = changesIds.sort((a, b) => a - b);
  let changesFlat = [...new Set(changesSorted)];
